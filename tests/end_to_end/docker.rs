...
impl Client {
    pub fn new() -> Self {
        let host = env::var("DOCKER_HOST")
            .ok()
            .map(|h| Url::parse(&h).unwrap_or_else(|_| panic!("Can't parse DOCKER_HOST: {h}")));

        match host {
            Some(x) if !matches!(x.scheme(), "unix" | "npipe") => Self {
                inner: Docker::connect_with_http_defaults().unwrap(),
                host: x.host_str().unwrap().to_owned()
            },
            Some(x) => Self {
    ...
    pub async fn build_local_image(
        &self, image: &str, tag: &str, mut dockerfile: File
    ) -> Result<(), bollard::errors::Error> {
        let mut tarball: Vec<u8> = Vec::new();
        tar::Builder::new(&mut tarball)
        ...
        while let Some(res) = stream.next().await {
            match res {
                Ok(x) => {
                    if let Some(s) = x.stream.filter(|s| s != "\n") {
                        eprintln!("Building image...{s}")
                    }
                }
                Err(e) => return Err(e)
            }
    ...
    pub async fn copy_file(
        &self, container_id: &str, mut src_file: File, dest_path: &Path
    ) -> Result<(), bollard::errors::Error> {
        let mut tarball: Vec<u8> = Vec::new();
        tar::Builder::new(&mut tarball)
            .append_file(dest_path.file_name().and_then(|x| x.to_str()).unwrap(), &mut src_file)
            .expect("Can't make tarball");
    ...
    fn docker_in_docker_host() -> anyhow::Result<String> {
        ...
        let cmd = Command::new("sh").arg("-c").arg("ip route|awk '/default/ { print $3 }'").output()?;
