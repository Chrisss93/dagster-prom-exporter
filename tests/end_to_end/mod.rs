...
pub async fn end_to_end(img_name: &str, img_tag: &str) {
    ...
    let image = GenericImage::new(img_name, img_tag)
        ...
        .with_wait_for(WaitFor::StdOutMessage {
            message: "Serving dagster-webserver on http://0.0.0.0:3000".to_owned()
        })
        .with_wait_for(WaitFor::Duration { length: Duration::from_millis(500) });

    let args = vec!["-f".to_owned(), "usercode.py".to_owned()];
    ...
    docker_client
        ...
        .await
        .expect("Failed to copy usercode file to container");

    let dagit_url = format!(
        "http://{}:{}/graphql",
        docker_client.host(),
        dagster.get_host_port_ipv4(3000).await
    );

    // Start the dagster prometheus exporter
    eprintln!("Dagster docker container: {}", dagster.id());
    let _exporter = spawn_local(dagster_prom_exporter::serve(
        dagit_url.clone(),
    ...
    assert!(
        load_usercode(http.post(&dagit_url)).await,
        "Dagster usercode cannot been loaded"
    );
    ...
    let run_ids = run_dagster_pipelines(
        http.post(&dagit_url),
        vec![
            Pipeline::new("hello_world", false, None),
            Pipeline::new("alphabet", true, None),
        ]
    )
    .await;
    ...
    let more_pipelines = vec![
        Pipeline::new("ascii_job", false, None),
        Pipeline::new(
            "hello_world",
            false,
            Some(r#"{"ops": {"world": {"config": {"name": "Chris"}}}}"#)
        ),
        Pipeline::new("alphabet", true, None),
    ];
...
fn test_completed_run_metrics(samples: &[prometheus_parse::Sample]) -> (f64, f64) {
    use prometheus_parse::Labels;
    use prometheus_parse::Value::{Gauge, Untyped};

    let run_total: Vec<&Labels> =
        samples.iter().filter(|x| x.metric == "run_duration_seconds").map(|x| &x.labels).collect();

    assert_eq!(
        run_total.len(),
        2,
        "Expected 2 samples for the run_total metric, got: {run_total:?}"
    );
    assert!(
        run_total.iter().all(
            |x| (x.get("pipeline_name") == Some("__ASSET_JOB") && x.get("status") == Some("FAILURE")) ||
    ...
    let duration: Vec<&Labels> =
        samples.iter().filter(|x| x.metric == "run_duration_seconds").map(|x| &x.labels).collect();

    assert_eq!(
        duration.len(),
        2,
        "Expected 2 samples for the run_duration metric, got: {duration:?}"
    );
    assert!(
        duration.iter().all(
            |x| (x.get("pipeline_name") == Some("__ASSET_JOB") && x.get("status") == Some("FAILURE")) ||
    ...
    let step_total: Vec<&Labels> = samples
        .iter()
        .filter(|x| x.metric == "step_total" && x.value == Untyped(1.0))
        .map(|x| &x.labels)
        .collect();
    ...
    assert_eq!(
        step_total.len(),
        4,
        "Expected 4 samples for the step_total metric, got: {step_total:?}"
    );
    assert!(
        step_total.iter().all(|x| {
    ...
    for metric in ["step_duration_seconds", "step_attempts"] {
        let m: Vec<&Labels> = samples
            ...
            .collect();
        assert_eq!(m.len(), 4, "Expected 4 positive samples for the {metric} metric, got: {m:?}");
        assert!(
            m.iter().all(|x| {
                let (a, b, c) = (x.get("pipeline_name"), x.get("step_key"), x.get("status"));
                ((a, b, c) == (Some("hello_world"), Some("hello"), Some("SUCCESS"))) ||
    ...
    assert!(
        {
            ...
            asset.labels.get("step_key") == Some("save_s3") &&
                match asset.value {
                    Gauge(x) => x > 0.0,
                    _ => false
                }
    ...
    let last_scraped: Vec<_> = samples
        .iter()
        .filter(|x| {
            x.metric == "exporter_last_scrape_runs" &&
                match x.value {
                    Gauge(i) => (i - 2.0).abs() < f64::EPSILON,
                    _ => false
                }
        })
        .collect();
    ...
    let last_scraped_ts: Vec<_> = samples
        .iter()
        .filter(|x| {
            x.metric == "exporter_last_scrape_timestamp" &&
                match x.value {
                    Gauge(i) => i > 0.0,
                    _ => false
                }
    ...
    assert_eq!(
        last_scraped_ts.len(),
        1,
        "Expected positive last scraped timestamp but got: {last_scraped_ts:?}"
    );

    match (
        assets.first().map(|x| x.value.clone()),
        last_scraped_ts.first().map(|x| x.value.clone())
    ) {
...
#[allow(clippy::too_many_lines)]
fn test_more_run_metrics(samples: &[prometheus_parse::Sample], asset_ts: f64, scrape_ts: f64) {
    use prometheus_parse::Labels;
    use prometheus_parse::Value::{Gauge, Untyped};
    ...
    assert_eq!(
        run_total.len(),
        4,
        "Expected 4 samples for the run_total metric, got: {run_total:?}"
    );

    assert!(
        run_total.iter().all(|x| {
    ...
    assert_eq!(
        duration.len(),
        3,
        "Expected 3 samples for the run_duration metric, got: {duration:?}"
    );
    assert!(
        duration.iter().all(
            |x| (x.get("pipeline_name") == Some("__ASSET_JOB") && x.get("status") == Some("SUCCESS")) ||
                (x.get("pipeline_name") == Some("hello_world") && x.get("status") == Some("SUCCESS")) ||
    ...
    assert_eq!(
        step_total.len(),
        6,
        "Expected 6 samples for the step_total metric, got: {step_total:?}"
    );
    ...
    assert!(
        step_total.iter().all(|x| {
            let (a, b, c, d) = (
                &x.value,
                x.labels.get("pipeline_name"),
                x.labels.get("step_key"),
                x.labels.get("status")
            );
    ...
    for metric in ["step_duration_seconds", "step_attempts"] {
        let m: Vec<&Labels> = samples
            ...
            .collect();
        assert_eq!(m.len(), 5, "Expected 5 positive samples for the {metric} metric, got: {m:?}");
        assert!(
            m.iter().all(|x| {
                let (a, b, c) = (x.get("pipeline_name"), x.get("step_key"), x.get("status"));
                ((a, b, c) == (Some("hello_world"), Some("hello"), Some("SUCCESS"))) ||
    ...
    assert!(
        expectations.iter().all(|x| x.value == Gauge(0.0) &&
            x.labels.get("step_key").filter(|y| y == &"world" || y == &"alphabet").is_some()),
    ...
    assert!(
        assets.iter().all(
            |x| ["s3/hello_world", "alphabet", "ascii"].map(Some).contains(&x.labels.get("asset_key")) &&
                match x.value {
                    Gauge(i) => i > asset_ts,
                    _ => false
                }
...
async fn parse_metrics(req: reqwest::RequestBuilder) -> anyhow::Result<Vec<prometheus_parse::Sample>> {
    ...
    let metrics = prometheus_parse::Scrape::parse(response.lines().map(|s| Ok(s.to_owned())))?;
    Ok(metrics.samples)
}
